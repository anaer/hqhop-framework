package com.hqhop.framework.common.orm.search.utils;
...
import com.hqhop.framework.common.orm.search.exception.InvalidSearchPropertyException;
import com.hqhop.framework.common.orm.search.exception.InvalidSearchValueException;
import com.hqhop.framework.common.orm.search.exception.SearchException;
import com.hqhop.framework.common.orm.search.filter.AndCondition;
import com.hqhop.framework.common.orm.search.filter.Condition;
import com.hqhop.framework.common.orm.search.filter.OrCondition;
import com.hqhop.framework.common.orm.search.filter.SearchFilter;
...
public final class SearchableConvertUtils {
	...
	/*
	 * public static <T> void convertSearchValueToEntityValue(SearchRequest
	 * search, Class<T> domainClass) { List<Condition> searchFilters =
	 * search.getSearchFilters(); for (Condition searchFilter : searchFilters) {
	 * String property = searchFilter.getSearchProperty(); Class<? extends
	 * Comparable> targetPropertyType = getPropertyType(domainClass, property);
	 * Object value = searchFilter.getValue(); Comparable newValue =
	 * convert(value, targetPropertyType); searchFilter.setValue(newValue); } }
	 */
	/*
	 * private static <T> Class getPropertyType(Class<T> domainClass, String
	 * property) { String[] names = StringUtils.split(property, "."); Class<?>
	 * clazz = null;
	 * 
	 * for (String name : names) { if (clazz == null) { clazz =
	 * BeanUtils.findPropertyType(name, ArrayUtils.toArray(domainClass)); } else
	 ...
	 * ConversionService conversionService = getConversionService(); if
	 * (!conversionService.canConvert(sourceValue.getClass(), targetClass)) {
	 * throw new IllegalArgumentException(
	 * "search param can not convert value:[" + sourceValue +
	 * "] to target type:[" + targetClass + "]"); }
	 * 
